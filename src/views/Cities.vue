  ...
  <div id="cities">
    <div id="list">
      ...
      <div class="adding-form-wrap">
        <form class="adding-form" @submit.prevent="addCity">
          ...
          <input
            type="text"
            name="population"
            placeholder="Miesto populiacija"
          />
          <input
            type="text"
            name="postal_code"
            placeholder="Miesto pašto kodas"
          />
      ...
      <TableContent
        ...
        :isCountry="false"
        @updated="getData(cities.meta.current_page)"
      ></TableContent>
    </div>
      ...
      if (this.isFirstGet) {
        try {
          const response = await this.$http.get(
            "https://akademija.teltonika.lt/countries_api/api/countries/cities?page=" +
              page
          );
      ...
      try {
        const response = await this.$http.post(
          "https://akademija.teltonika.lt/countries_api/api/countries/" +
            submitEvent.target.elements.id.value +
            "/cities",
          {
            data: {
              attributes: {
